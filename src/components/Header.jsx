...
import { toggleNavigationalPanel } from "../store/appSlice";
import { useEffect, useState } from "react";
import { SEARCH_SUGGESTIONS } from "../utils/constants";
import { addCache } from "../store/videoSlice";
...
const Header = () => {
  ...
  const cache = useSelector((store) => store.video.suggestionsCache);
  ...
  const fetchSuggestions = async (query, signal) => {
    if (query) {
      ...
      try {
        const response = await fetch(SEARCH_SUGGESTIONS + query, { signal });
        if (!response.ok) {
          throw new Error("Response issue");
        }
        const data = await response.json();
        setSuggestions(data[1]);
        dispatch(addCache({ [query]: data[1] }));
      } catch (e) {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      timerId = setTimeout(() => {
        fetchSuggestions(searchQuery, signal);
      }, 200);
    } else {
      setSuggestions([]);
    }
  ...
  return (
    <header className="flex py-4 px-6 items-center justify-between h-[68px]">
      <section className="flex gap-4 items-center">
        <button
          onClick={() => {
            dispatch(toggleNavigationalPanel());
          }}
        >
          <MenuIcon
            fontSize="large"
        ...
        <Link to="/">
          <div className="cursor-pointer flex">
            ...
            <span className="text-gray-500 ml-1 text-xs">{countryCode}</span>
          </div>
        </Link>
      </section>
      <section>
        <section className="flex items-center">
          <input
            ...
            placeholder="Search"
            value={searchQuery}
            className="border border-gray-400 py-1 px-4 w-[28rem] rounded-l-full"
            onChange={(e) => {
          ...
          />
          <button className="border border-gray-400 rounded-r-full border-l-0 py-[2px] px-3 bg-gray-100">
            <svg
              width="28px"
              height="28px"
        ...
        {showSuggestion && suggestions.length > 0 && (
          <section className="fixed bg-white w-[28rem] my-1 shadow-lg rounded-lg max-h-72 overflow-y-auto custom-scrollbar">
            <ul>
              {suggestions.map((suggestion) => (
                <li
                  key={suggestion}
                  className="my-2 py-1 px-6 cursor-pointer hover:bg-gray-300 font-semibold"
                >
                  {suggestion}
                </li>
              ))}
